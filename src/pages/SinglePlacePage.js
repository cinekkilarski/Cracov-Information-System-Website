...
class SinglePlacePage extends Component {
    ...
    handleStarRating = (rate) => {
        ...
        if (Cookies.get("token")) {
            ...
            fetch(`http://localhost:8080/api/places/explore/${this.props.match.params.id}/updaterate`, {
                method: 'PUT',
    ...
    handleAddComment = (e) => {
        ...
        setTimeout(() => { this.childComments.current.handleLoadComments() }, 500)
    }


    componentDidMount() {
        // console.log(this.props.match.params.id);
        fetch(`http://localhost:8080/api/places/explore/${this.props.match.params.id}`)
            .then(res => res.json())
            .then(res => {
                this.setState({
    ...
    render() {
        // console.log(this.state.exploreData);
        return this.state.isLoaded && (
            <div className="backgroundSingle">
                <img className="imgPlace" src={this.state.exploreData.image} alt="place" />
                ...
                <div className="mainPlaceDataContainer">
                    ...
                    <Card className="rateCss">
                        ...
                        <StarRating className="starRating" handleStarRating={this.handleStarRating} grade={this.state.averageGrade} />
                    </Card>
                    ...
                    <Card className="commentCss">
                        ...
                        <Comments currentPlace={this.props.match.params.id} ref={this.childComments} />
                        <div>
                            <form className='addCommentCss' onSubmit={this.handleAddComment}>
                                <textarea type="text" id="text"
                                    name="text" placeholder="Add your opinion" value={this.state.text} onChange={this.handleInputChange}></textarea>
                ...
                <div className="mapPanel">
                    <OpenStreetMap exploreData={this.state.exploreData} ref={(cd) => this.child = cd} />
                </div>
                <Modal
                    variant="warning"
                    size="sm"
                    show={this.state.displayMessage}
                    onHide={() => { this.setState({ displayMessage: false }) }}
                    ...
                    <div className="modalStyle">
                        ...
                        <Modal.Body>{this.state.successMessage ? this.messages.successMessage : this.messages.loginMessage}</Modal.Body>
                    </div>
                </Modal>
            </div >
        );
